...
� **استخدم get_smart_response() عند الحاجة لتحليل سياق الرسالة وتقديم رد ذكي مناسب**
...
**👋 ردود السلامات:**
أرد بحرارة على السلامات وأعرض المساعدة في التأمين والخدمات.
...
**⚠️ قوانين التحقق:**
أستخدم طرق التحقق المخصصة لكل نوع بيانات (الاسم، الهوية، الجوال، العمر) وأخلي الطريقة تتحقق من صحة البيانات.
...
"""
    ...
    def _smart_greeting(self) -> str:
        ...
        if is_returning:
            return f"""وعليكم السلام ورحمة الله وبركاته! 🌟 أهلاً وسهلاً فيك مرة ثانية في {company_name}.
...
إيش أقدر أساعدك فيه اليوم؟"""
            ...
            return f"""وعليكم السلام ورحمة الله وبركاته! 👋

أهلاً وسهلاً فيك في {company_name}! أنا مساعدك الذكي ومبسوط إني أخدمك.
